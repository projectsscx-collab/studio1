 ...
 * This flow is called in the final step to update the opportunity with its final status and policy info.
 */
...
const getSalesforceTokenForOppUpdateFlow = ai.defineFlow(
  {
...
const updateOpportunityFlow = ai.defineFlow(
  ...
  async ({ opportunityId, payload }) => {
    const token = await getSalesforceTokenForOppUpdateFlow();
    
    // The standard Salesforce REST API endpoint for updating an Opportunity record.
